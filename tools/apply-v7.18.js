...
if (!s.includes(oldBlock)) {
  console.error('No encontré el bloque favorites esperado.');
  process.exit(2);
}

s = s.replace(oldBlock, newBlock);

if (!s.includes("const CACHE_MINUTES = 1440; // 24 horas")) {
  console.error('No encontré CACHE_MINUTES.');
  process.exit(3);
}
...
if (!s.includes("const MODEL_VERSION = 'V7.17.0';")) {
  console.error('No encontré MODEL_VERSION.');
  process.exit(4);
}
...
s = s.replace(
  "MK Bets V7.17.0 - Pronósticos Deportivos",
  "MK Bets V7.18.0 - Pronósticos Deportivos"
);
